...
Baseline of the test using the default implementation of the Honest Profiler finished the test in 74.723 milliseconds on average. During profiling, 19025 samples were obtained on average from which 15130 (81\%) contained frames of interest.
Profiling results are visualized on the figure ~\ref{fig:baseline} on page ~\pageref{fig:baseline}.
\TODO{Before shipping final, verify that the picture is positioned well}
\TODO{Is defining test machine specs necessary?}

\begin{figure}[H]
\includegraphics[scale=0.32]{baseline.png}
\caption{Visualization of basline benchmark profiling results}
\label{fig:baseline}
\end{figure}
...
\TODO{results}
\TODO{why shared mem? io too expensive}
