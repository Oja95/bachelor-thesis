...
%\supervisor{The first part of the sentence does not connect very well with the second. Maybe it is because of ``results'' and ``tools'' being generic terms whose meaning one has to parse with context in mind. Reads have only one paragraph worth of context by time they read this} \TODO{Replaced 'result' with 'outcome'. Better?}
%As the profiling outcome relies on the efficiency and accuracy of the profiling tools used,  it is important to have sufficient and actionable information from the profiler to construct accurate profiles of the application's performance. 
...
%\supervisor{Bit thorn here, we are guaranting that increased samples increases accuracy, but this might be overselling it. At same time, where else to oversell if not in the introduction? Using some sort of conditonal there would make it much weaker. Maybe ask for a second opinion from someone on this.} 
%\TODO{Added 'potentially', is better?}
% Promised potential increase in accuracy, hopefully not overselling it this way.
