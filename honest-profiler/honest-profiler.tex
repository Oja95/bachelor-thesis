...
Therefore, increasing the sampling ratio will require significant changes in the architecture of how timer signals are sent to the profiler. As we shall see, different architectures not only perform differently in terms of overhead but also in terms of which thread ends up receiving timer signals. Hence, the ratio of usable stack traces versus those that have to be discarded are different for each implementation.
