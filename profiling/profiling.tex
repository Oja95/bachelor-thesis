...
\supervisor{Maybe a quick paragraph or two that starts of what problem we try to solve by profiling at all (identifing preformance issues)? To provide context and make it easier to ``sell'' why one should bother reading the following.}

Sampling profilers gather call traces from the observable program at regular intervals. \supervisor{Does not have to be regular intervals, as you talk later on, just scrap the world regular?} Distribution of the gathered samples highlights the hotspots in the observable program. Higher frequency of a call trace suggests that more program's execution time was spent in that particular state. Profiling result is a statistical approximation of the program's performance. Thus, having more samples will provide a more accurate approximation. This approach also assumes that samples are gathered randomly as bias in the samples could potentially yield inaccurate results. 


Figure ~\ref{fig:samplingProf} on page ~\pageref{fig:samplingProf} helps to visualize the performance implications of the frequency of the gathered samples. The graph visualizes the performance outline of a simple program that executes two identical methods, \texttt{methodA} and \texttt{methodB}, alternately. This simple program executed for $911$ milliseconds during which $120$ usable samples were collected. $58$ of those samples contained \texttt{methodA} in its top call frame and $44$ contained \texttt{methodB} in its top frame. \supervisor{I think it is worth mentioning passingly that flamegraph is the way we will use to vizualise profiling data and provide a citation for futher information. Flamegraph is not a very common way of displaying profile data, so although it is quite intuitive, we should offer a ``definiton''. Maybe worth mentoning that otherwise, profiling data is just a bunch of stacktraces with number of times they have been seen.}

Although sampling profiling does not provide precise time metrics for each method, it does \supervisor{does is strong, can provide might be better :D} provide a general overview to visualize the time spent in the profileable application's context.

\supervisor{Let's mention the most common alternative to sampling briefly and discuss the pros of sampling based profilers? Instrumentation based profiling that is. An idea, maybe out of scope.}
\begin{figure}[H]
...
\supervisor{This need some expanding, to covince reader that increasing the sample size is a worthy goal. Think we should put some writing effort into this. For me, the motivation was that I could not get a decent picture from honest profiler about multilpe relatively short methods. Getting 1-2 samples per some of them does not really allow one to focus down on some of the methods. What makes it worse is that the profiling results differ from run to run, making them pretty much useless.}

\supervisor{More convinced that we need to bring out pros of sampling profiler, as otherwise one might ask, why bother with this challenged approach?}
