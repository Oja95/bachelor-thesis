...
\end{lstlisting}
As various Java Virtual Machine routines are nondeterministic, it is impossible to predict when does the Java Virtual Machine signal its threads to be stopped on a safepoint. However, if such request should occur in the middle of the outer counted loop's execution in Listing \ref{lst:safepoint}, the thread executing the outer loop's instructions will not stop until the loop has finished. Thus, if an ordinary sampling profiler signals the thread for a call trace sample, it is delayed until the thread executing the outer loop's instructions finishes its task \cite{wakart_psychosomatic_2015}. 
