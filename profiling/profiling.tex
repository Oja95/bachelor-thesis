...
\TODO{Cover instrumentation based tracing profiling approach briefly}
\TODO{Pros of sampling profiling}
...
\TODO{Should citation be at the end of paragraph?}
...
Although the provided example is rather artificial and such occurrences in real life programs are rare, it clearly points out the bias that the safepoints potentially introduce. Such bias during obtaining the call trace clearly contradicts the randomness prerequisite for sampling profiling.


\TODO{Ideas to expand on:}
Introduce Vootele's idea that safepoints won't turn profiling results into gibberish but rather lessens the accuracy when high granularity matters
